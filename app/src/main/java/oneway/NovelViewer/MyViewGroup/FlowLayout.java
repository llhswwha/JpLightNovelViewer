...
public class FlowLayout extends ViewGroup {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec)
    {
        ...
        for (int i = 0; i < cCount; i++)
        {
            ...
            // 当前子空间实际占据的宽度
            int childWidth = getChildWidth(child) + lp.leftMargin + lp.rightMargin;
            // 当前子空间实际占据的高度
            int childHeight = getChildHeight(child) + lp.topMargin + lp.bottomMargin;
            /**
             * 如果加入当前child，则超出最大宽度，则的到目前最大宽度给width，类加height 然后开启新行
             */
            if (isWrap(child)||lineWidth + childWidth > sizeWidth)
            {
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b)
    {
        ...
        for (int i = 0; i < cCount; i++)
        {
            ...
            // 如果已经需要换行
            if (isWrap(child)||childWidth + lp.leftMargin + lp.rightMargin + lineWidth > width)
            {
                // 记录这一行所有的View以及最大高度
        ...
        for (int i = 0; i < lineNums; i++)
        {
            ...
            for (int j = 0; j < lineViews.size(); j++)
            {
                ...
                int rc =lc + getChildWidth(child);
                int bc = tc + getChildHeight(child);
                ...
                left += getChildWidth(child) + lp.rightMargin
                        + lp.leftMargin;
            }
            left = 0;
            top += lineHeight;
    ...
    protected int getChildWidth(View childView)
    {
        if(isWrap(childView))
        {
            return 0;
    ...
    protected int getChildHeight(View childView)
    {
        if(isWrap(childView))
        {
            return 0;
